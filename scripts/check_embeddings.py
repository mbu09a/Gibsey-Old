...
try:
    ...
    embedding_count_result = sb.table("pages").select("count", count="exact").not_.is_("embedding", "null").execute()
    ...
    null_embedding_count_result = sb.table("pages").select("count", count="exact").is_("embedding", "null").execute()
    ...
    if total_rows == 33:
        print(f"4. Total confirms expected 33 shards: ✅")
    else:
