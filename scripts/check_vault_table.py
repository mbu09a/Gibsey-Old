...
try:
    ...
    if total_vault_rows > 0:
        ...
        if 'embedding' in sample_row.data[0].keys():
            vault_with_embeddings = sb.table("vault").select("count", count="exact").not_.is_("embedding", "null").execute()
...
try:
    ...
    tables_result = sb.table("pg_catalog.pg_tables").select("tablename").eq("schemaname", "public").execute()
