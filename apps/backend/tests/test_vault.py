...
def test_list_vault_entries(mocker):
    ...
    mock_supabase.table.return_value.select.assert_called_once_with("id, question, answer, created_at")
    mock_supabase.table.return_value.select.return_value.order.assert_called_once_with("created_at", desc=True)
    mock_supabase.table.return_value.select.return_value.order.return_value.range.assert_called_once_with(0, 1)  # 0 to (0+2-1)
...
def test_list_vault_entries_error_handling(mocker):
    ...
    mock_supabase = mocker.MagicMock()
    mock_supabase.table.return_value.select.return_value.order.return_value.range.return_value.execute.side_effect = Exception("Database error")
    mocker.patch("app.main.Supabase.client", return_value=mock_supabase)
